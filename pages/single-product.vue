<template>
 ...
 <v-tabs>
            ...
            <v-tab-item class="pa-6">
             ...
             <v-tab-item class="pa-6">
              <div v-html="product.how_to_use || ''">

              </div>
            </v-tab-item>
            <v-tab-item class="pa-6">
              <div v-html="product.ingridient || ''">
            ...
            <v-tab-item class="pa-6">
              ...
              <div v-for="(i, index) in product.reviews" :key="index" style="border-bottom: 1px solid #ddd;" class="py-4">
              ...
              <v-row class="reviews mt-10">
                ...
                <v-col >
                  ...
                  <v-form v-model="valid" ref="form" lazy-validation>
                    <v-textarea v-model="form.content" required :rules="[v=>!!v || 'This field is required']" placeholder="Write your review here" outlined></v-textarea>
                    <v-row>
                      <v-col cols="12" md="6">
                        <v-text-field v-model="form.name" required :rules="[v=>!!v || 'This field is required']" placeholder="Your name" outlined>

                        </v-text-field>
                      </v-col>
                      <v-col md="6">
                        <v-text-field v-model="form.email" required :rules="[v=>!!v || 'This field is required']" placeholder="Your email" outlined>

                        </v-text-field>
                      </v-col>
                    </v-row>
                    <v-btn class="secondary" :loading="loading" @click="$refs.form.validate() ? submitReview() : null" block text x-large>Submit Review</v-btn>
                  </v-form>
                </v-col>
              </v-row>
            </v-tab-item>
...
export default {
  ...
  methods: {
    ...
    async getRelatedProduct () {
      ...
      await this.$store.dispatch('products/relatedproduct', data).then(response => {
...
async submitReview () {
  this.loading = true
      ...
      await this.$store.dispatch('products/addreview', this.form).then(response => {
