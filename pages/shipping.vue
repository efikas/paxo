<template>
  <v-container>
    ...
    <v-form lazy-validation v-model="valid" ref="form">
      <v-row class="mt-4 mb-10" :class="{ 'px-4': $vuetify.breakpoint.smAndDown, 'p10p': $vuetify.breakpoint.mdAndUp }">
        <v-col md="6" cols="12">
          ...
          <div class="mt-4"> <v-btn outlined small color="primary" class="primary-text mb-2 br-all-5 text-caption"
              to="/checkout">
              Change</v-btn>
          </div>
          ...
          <div class="contact-cardj pa-4l">
            <v-radio-group v-model="paymentoption" required
              ...
              <v-radio label="Pay with your wallet" :disabled="user.balance == '0'" value="1">
                <template v-slot:label>
                  <div>Pay with your wallet <div class="text--disabled text-caption">Wallet Balance: &#8358;{{
                    user.balance | formatPrice }}</div>
                  </div>
                </template>
              </v-radio>
            ...
            <v-form lazy-validation ref="coupon" v-model="valid">
              <v-row class="pr-0 pl-0">
                <v-col cols="8" md="9" class="pr-0 pl-0">
                  <v-text-field outlined label="Enter Coupon Code" v-model="code" required
                    :rules="[(v) => !!v || 'Please enter coupon code']"></v-text-field>
                </v-col>
                <v-col cols="3" md="3">
                  <v-btn :loading="loading" @click="$refs.coupon.validate() ? applyCoupon() : null" primary
                    color="primary" class="br-all-5" large block height="56">Apply code</v-btn>
                </v-col>
              </v-row>
              <div class="mt-2">
                <p class="py-0 my-0 font-weight-medium">Hot to get a Coupon Code?</p>
                <p class="py-0 mt-2">Follow our news on the website, as well as subscribe to our social networks. So you
                  will not
                  only be able to receive up-to-date codes, but also learn about new products and promotional items.</p>
              </div>
            </v-form>
          </div>
        </v-col>
        <v-col md="6">
          <div class="checkout-order">
            ...
            <div v-for="(i, index) in StoreCart.filter(
            ...
            <div class="shopping-total pa-10">
              ...
              <v-btn v-if="paymentoption == '0'" class="primary black--text br-all-5" large block text
                :loading="loading" @click="$refs.form.validate() ? (confirmDialog = true) : null"><span
                  style="color: white">Complete
                  Order</span></v-btn>

              <v-btn v-if="paymentoption == '2'" class="primary black--text br-all-5" large block text
                :loading="loading" @click="$refs.form.validate() ? (confirmDialog = true) : null"><span
                  style="color: white">Complete
                  Order</span></v-btn>

              <v-btn v-if="paymentoption == '1'" class="primary black--text br-all-5" large block text
                :loading="loading" @click="$refs.form.validate() ? (walletDialog = true) : null"><span
                  style="color: white">Complete
                  Order</span></v-btn>
            </div>
          </div>
    ...
    <paystack :amount="order.order_balance * 100" :email="user.email" :paystackkey="paystackkey" :reference="reference"
      :callback="callback" :channels="channels" :close="close" :embed="false" id="paystack" style="visibility: hidden">
      <i class="fas fa-money-bill-alt"></i> Make Payment
    </paystack>
    <v-dialog width="400" v-model="confirmDialog" overlay-color="#141515" overlay-opacity="0.9">
      <div class="pa-0 ma-0 br-all-10">
        <div class="text-center py-4" style="background-color: #00C3B7;">
          <h3 style="color:white;">
            Order Total:
          </h3>
          <h4 style="color:white;" class="font-weight-medium">&#8358;{{ (subtotal + parseInt(user.deliveryfee)) |
            formatPrice }}</h4>
        </div>
        <div class="pa-6 text-center" style="background-color: white;">
          ...
          <v-btn class="primary br-all-5" :loading="loading" @click="createOrder()">Proceed</v-btn>
        </div>
      </div>
    </v-dialog>
    <v-dialog width="470" v-model="walletDialog" overlay-color="#141515" overlay-opacity="0.9">
        ...
        <div class="pa-6 text-center" style="background-color: white;">
          ...
          ">
            <v-col class="pb-0 text-center">
              <v-btn outlined text small class="br-all-5">Wallet Payment: &#8358;{{ user.balance | formatPrice
                }}</v-btn>
            </v-col>
            <v-col ma-0>
              <v-btn class="primary br-all-5" small>Card Payment: &#8358;{{
          ...
          <v-btn class="primary br-all-5" :loading="loading" @click="createOrder2()">Proceed</v-btn>
        </div>
      </div>
    </v-dialog>
  </v-container>
