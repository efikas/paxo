...
export const actions = {
  ...
  async makeorder(
    { commit },
    { shipping_id, address, city, state, set_paid, product,email, phone, use_wallet, total, total_product, country, device, delivery_method }
  ) {
    let token = JSON.parse(window.localStorage.getItem('paxo')).auth.token
    const data = await this.$axios.$post(
      '/user/order/create',
      {
        ...
        product,email,phone,
        use_wallet,
        total, total_product, country, device, delivery_method, state
      },
      {
        headers: {
          Authorization: 'Bearer ' + token,
