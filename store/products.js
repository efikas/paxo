...
export const actions = {
  ...
  async makeorder(
    { commit },
    { shipping_id, address, city, state, lga, set_paid, product,email, phone, use_wallet, total, total_product, country, device, delivery_method, code }
  ) {
    let token = JSON.parse(window.localStorage.getItem('paxo')).auth.token
    const data = await this.$axios.$post(
      '/user/order/create',
      {
        ...
        product,email,phone,
        use_wallet,
        total, total_product, country, device, delivery_method, state, code, lga
      },
      {
        headers: {
          Authorization: 'Bearer ' + token,
